...
(i),'Y');}A(Y,X,'@');refresh();c=getchar();while(c!='Q'){CT;A(Y,X,'.');if(c=='w'){if(CE(Y-1,X)){H(Y
-1,X,es,&ch);}EI(CW(Y-1,X)){WH;AH;}else{--Y;AH;}}EI(c=='a'){if(CE(Y,X-1)){H(Y,X-1,es,&ch);}EI(CW(Y,
X-1)){WH;AH;}else{--X;AH;}}EI(c=='s'){if(CE(Y+1,X)){H(Y+1,X,es,&ch);}EI(CW(Y+1,X)){WH;AH;}else{++Y;
AH;}}EI(c=='d'){if(CE(Y,X+1)){H(Y,X+1,es,&ch);}EI(CW(Y,X+1)){WH;AH;}else{++X;AH;}}EI(c=='.'){AH;}F(
0,3,i){A(EY(i),EX(i),'.');if(((X==EX(i)-1||X==EX(i)+1)&&Y==EY(i))||((Y==EY(i)-1||Y==EY(i)+1)&&X==EX
(i))){--PH;}else{if(X>EX(i)){if(CE(EY(i),EX(i)+1)!=true&&CW(EY(i),EX(i)+1)!=true){++EX(i);}}EI(X<EX
(i)){if(CE(EY(i),EX(i)-1)!=true&&CW(EY(i),EX(i)-1)!=true){--EX(i);}}EI(Y>EY(i)){if(CE(EY(i)+1,EX(i))
!=true&&CW(EY(i)+1,EX(i))!=true){++EY(i);}}EI(Y<EY(i)){if(CE(EY(i)-1,EX(i))!=true&&CW(EY(i)-1,EX(i))
!=true){--EY(i);}}}A(EY(i),EX(i),'Y');}if(PH<=0){PS(5,3,"You Loose! Press Q");getch();break;}if(ch.g
>=20){PS(5,3,"You Win! Press Q");getch();break;}A(Y,X,'@');UH(PH,ch.g);refresh();c=getchar();}endwin
();return 0;}
