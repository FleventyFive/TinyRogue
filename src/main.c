...
void H(int y,int x,E*es,_C*c) {
  for(int i=0;i<3;++i) {
    if(EX(i)==x&&EY(i)==y) {
      int d=rand()%5;
      es[i].h-=d;
      PS(1,31,"Dealt %d damage",d);
      if(es[i].h<=0) {
        ++c->g;
        A(EY(i),EX(i),'.');
        es[i].h=5;
...
int main(void) {
  ...
  while(c!='Q') {
    ...
    F(0,3,i) {
      A(EY(i),EX(i),'.');
      if(((X==EX(i)-1||X==EX(i)+1)&&Y==EY(i))||((Y==EY(i)-1||Y==EY(i)+1)&&X==EX(i))) {
        --PH;
      } else {
        if(X>EX(i)) {
          if(CE(EY(i),EX(i)+1)!=true&&CW(EY(i),EX(i)+1)!=true) {
            ++EX(i);
          }
        } EI(X<EX(i)) {
          if(CE(EY(i),EX(i)-1)!=true&&CW(EY(i),EX(i)-1)!=true) {
          ...
          if(CE(EY(i)+1,EX(i))!=true&&CW(EY(i)+1,EX(i))!=true) {
            ++EY(i);
          }
        } EI(Y<EY(i)) {
          if(CE(EY(i)-1,EX(i))!=true&&CW(EY(i)-1,EX(i))!=true) {
